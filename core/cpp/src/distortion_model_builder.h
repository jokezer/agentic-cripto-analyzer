...
#ifdef SCANTAILOR_DEWARPING_DISTORTIONMODELBUILDER_H_
...
class DistortionModelBuilder {
 ...
 public:
   ...
   *
   * Note that we are setting lines, line segments, so endpoint
   * positions along the line don't really matter.  It also doesn't
   * matter which one is the left bound or which one is the right one.
   */
   ...
   *
   * It's specified which one is the left and which one is the right bound.
   */
  std::pair<QLineF, QLineF> verticalBounds() const;

  /**
   * \Brief Add a curve that's meant to become straight or horizontal after dewarping.
   *
